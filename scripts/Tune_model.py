...
parser.add_argument(
    ...
    type=int,
    default=5,
    help="Encoding dimension for the autoencoder (default 5).",
)
...
H2_MIN, H2_MAX, H2_STEP = 32, 320, 16
H3_MIN, H3_MAX, H3_STEP = 32, 256, 16
LR_MIN, LR_MAX = 1e-4, 5e-3
LDEC_VALUES = [0.0, 0.1, 0.5, 1.0, 2.0, 4.0, 10.0, 50.0, 100.0]
...
if args.outer_fold in [1, 2, 3, 4, 5]:
    nested_root = code_dir / "output" / f"nested_cv_ae_optuna_7layers_dim{encoding_dim}" / f"fold_{args.outer_fold}"
else:
    nested_root = code_dir / "output" / f"nested_cv_ae_optuna_7layers_dim{encoding_dim}"
    ...
    def make_objective(X_outer_train_val, search_space_mode="wide", base_params=None):
        def objective(trial):
            ...
            if search_space_mode == "wide":
                ...
                h3 = trial.suggest_int("h3", H3_MIN, H3_MAX, step=H3_STEP)
                lr = trial.suggest_float("lr", LR_MIN, LR_MAX, log=True)
                lambda_decorr = trial.suggest_categorical(
                    "lambda_decorr", LDEC_VALUES
                )
                ...
                h3_low, h3_high = _clip_int_range(
                    h3_center, H3_MIN, H3_MAX, H3_STEP
                )

                h1 = trial.suggest_int("h1", h1_low, h1_high, step=H1_STEP)
                h2 = trial.suggest_int("h2", h2_low, h2_high, step=H2_STEP)
                h3 = trial.suggest_int("h3", h3_low, h3_high, step=H3_STEP)
